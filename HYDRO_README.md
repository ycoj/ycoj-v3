> 本文档是 Hydro 后端的原始 README，仅适用于本仓库根目录的 Hydro 后端代码，不代表整个 ycoj-v3 仓库的约定。
> 前端（`ycoj-ui/`，MIT）及其文档见 [ycoj-ui/README.md](ycoj-ui/README.md)；仓库总览见 [README.md](README.md)。
...
[中文文档](https://hydro.js.org/) / [English](./HYDRO_README_EN.md)  
...
1. 不可移除本项目的版权声明与作者/来源署名；（[AGPL3 7(b)](./HYDRO_LICENSE#L356)）
2. 当重分发经修改后的本软件时，需要在软件名或版本号中采用可识别的方式进行注明；（[AGPL3 7(c)](./HYDRO_LICENSE#L360)）
3. 除非得到许可，不得以宣传为目的使用作者姓名；（[AGPL3 7(d)](./HYDRO_LICENSE#364)）
...
参照 [HYDRO_CONTRIBUTING.md](./HYDRO_CONTRIBUTING.md)
