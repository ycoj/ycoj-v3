...
1. 暂停两个旧仓库的 `master` 新提交。切换前用 `git ls-remote` 核对 `ycoj/YCOJ` 的 `master` 仍为 `012eb47c058ab55daa4d972929f555a3cb25512e`，`ycoj/ycoj-ui` 的 `master` 仍为 `7af0f15c1699e4708e4c513aaf06e8b15f7c707c`。若有变化，先补迁新提交并重新验证。
2. 记录两个现有服务的工作目录、启动命令、PM2 配置及环境变量，备份未纳入 Git 的配置文件。保留旧检出和当前进程，作为回退点。
3. 确认新仓库 `master` 的 GitHub Actions 已通过，Git LFS 文件和后端子模块可读取。不要在切换生产前归档旧仓库。
4. 选择一个新的绝对路径作为统一检出目录；下面用 `/srv/ycoj-v3` 举例。部署账户需有 GitHub、Git LFS 及两个子模块的读取权限。
